...
public class RaccordementProvider extends ContentProvider {
	...
	public static final Uri CONTENT_URI = Uri
			.parse("content://com.inodex.inoprod.business.raccordement");
	...
	private static class DatabaseRaccordement extends SQLiteOpenHelper {
		...
		DatabaseRaccordement(Context context) {
			super(context, RaccordementProvider.CONTENT_PROVIDER_DB_NAME, null,
					RaccordementProvider.CONTENT_PROVIDER_DB_VERSION);
		}
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE "
					...
					+ " STRING," + Raccordement.REFERENCE_FABRICANT2
					+ " STRING," + Raccordement.REFERENCE_INTERNE + " STRING,"
					+ Raccordement.REFERENCE_OUTIL_ABOUTISSANT + " STRING,"
					+ Raccordement.REFERENCE_ACCESSOIRE_OUTIL_ABOUTISSANT
					+ " STRING," + Raccordement.REFERENCE_OUTIL_TENANT
					+ " STRING,"
					+ Raccordement.REFERENCE_ACCESSOIRE_OUTIL_TENANT
					+ " STRING," + Raccordement.REGLAGE_OUTIL_TENANT
					+ " STRING," + Raccordement.REPERE_ELECTRIQUE_ABOUTISSANT
					+ " STRING," + Raccordement.REPERE_ELECTRIQUE_TENANT
					+ " STRING," + Raccordement.REGLAGE_OUTIL_ABOUTISSANT
					+ " STRING," + Raccordement.REPRISE_BLINDAGE + " STRING,"
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		if (id < 0) {
			return db
					.query(RaccordementProvider.CONTENT_PROVIDER_TABLE_NAME,
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		try {
			...
			long id = db.insertOrThrow(
					RaccordementProvider.CONTENT_PROVIDER_TABLE_NAME, null,
					values);

			if (id == -1) {
				throw new RuntimeException(String.format(
						"%s : Failed to insert [%s] for unknown reasons.",
						"RaccordementProvider", values, uri));
			} else {
